...
void Blowfish_Encrypt_String(BLOWFISH_CTX *ctx,unsigned char *buffer, unsigned int size)
{
	unsigned int Length;
	int i,j;
	...
	for (i=0; i < Length; i = i + 8)
	{
		...
		for ( j= 0; j < 4; j++)
	      	{
			datal = (datal<<8) |buffer[i + j];
	      	}

		datar = 0x00000000;
		for ( j= 0; j < 4; j++)
	      	{
			datar = (datar<<8) |buffer[i + j +4];
	      	}
		Blowfish_Encrypt(ctx, &datal, &datar);

		for ( j= 0; j < 4; j++)
	      	{
			buffer[i + j] = datal>>(24-8*j);
	      	}

		for ( j= 0; j < 4; j++)
	      	{
			buffer[i + j + 4] = datar>>(24-8*j);
	      	}
	...
	if (Remain != 0)
	{
		for(i=0; i<Remain; i++)
		{
		       if (i >= 4)
			{
				datar = (datar<<8) |buffer[Length + i];
			}
			else
			{
				datal = (datal<<8) |buffer[Length + i];
			}		
		}
		Blowfish_Encrypt(ctx, &datal, &datar);
	      for ( j= 0; j < 4; j++)
	    	{
			buffer[Length + j] = datal>>(24-8*j);
	      	}
			
		for ( j= 0; j < 4; j++)
	      	{
			buffer[Length + j + 4] = datar>>(24-8*j);
	      	}
	}
#endif
}
...
void Blowfish_Decrypt_String(BLOWFISH_CTX *ctx, unsigned char *buffer, unsigned int size)
{
	unsigned int Length;
	int i,j;
	unsigned long datal, datar;
	...
	for (i=0; i < Length ; i = i + 8)
	{
		...
		for ( j= 0; j < 4; j++)
	      	{
			datar = (datar<<8) |buffer[i + 4 + j];
	      	}
		
		Blowfish_Decrypt(ctx, &datal, &datar);
#if 0		
		if (((size%8)!=0)&((i+8)==Length))
		{
			if ((size%8)<4)
				{
					datal = datal << (8*(4-size%8)) ;
				}
			else
				{
					datar = datar << (8*(8-size%8)) ;
				}
